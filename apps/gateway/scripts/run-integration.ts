...
// The dashboard surface is off by default in production; the integration suite exercises it, so it is
// switched on here with a deterministic root operator.
process.env.DASH_ENABLED ??= "true";
process.env.DASH_ROOT_USER ??= "integration-root";
process.env.DASH_ROOT_PASSWORD ??= "integration-root-password";
