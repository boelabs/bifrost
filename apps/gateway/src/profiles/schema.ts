...
/**
 * Per-deployment execution deadlines. Every bound is optional and may only tighten the router's
 * policy, so the schema needs no upper limit of its own: the resolver clamps against the global
 * value, which is where the administrative ceiling already lives.
 */
const executionPolicyOverrideSchema = z
	.object({
		firstOutputMs: z.int().positive(),
		idleMs: z.int().positive(),
		reasoningOnlyMs: z.int().positive(),
		preCommitMs: z.int().positive(),
		totalMs: z.int().positive(),
	})
	.partial()
	.strict();

export const executionPolicyOverridesSchema = z.partialRecord(
	z.enum([...CALL_TYPE_VALUES, "all"]),
	executionPolicyOverrideSchema,
);
