...
test("Responses reasoning parts keep identity, boundaries and plain-text separation in Chat", async () => {
	...
	assert.deepEqual(
		rendered.map((chunk) => chunk.choices[0]?.delta.provider_specific_fields),
		[
			{
				responses: {
					reasoning_item_id: "rs_1",
					reasoning_part: { id: "rs_1", index: 0, status: "streaming" },
				},
			},
			{
				responses: { reasoning_part: { id: "rs_1", index: 0, status: "done" } },
			},
			{
				responses: {
					reasoning_item_id: "rs_1",
					reasoning_part: { id: "rs_1", index: 1, status: "streaming" },
				},
			},
			{
				responses: { reasoning_part: { id: "rs_1", index: 1, status: "done" } },
			},
			{
				responses: {
					reasoning_item_id: "rs_2",
					reasoning_part: { id: "rs_2", index: 0, status: "streaming" },
				},
			},
			{
				responses: { reasoning_part: { id: "rs_2", index: 0, status: "done" } },
			},
		].map((openai) => ({ openai })),
	);
});
