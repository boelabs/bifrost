...
export function toOpenAIChatChunk(
	...
	return {
		...
		choices: chunk.choices.map((c) => {
			let providerSpecificFields = withoutOpenAIResponsesStreamMetadata(
			...
			if (
				...
				const part = {
					...
					index: Number(
						event.data.summary_index ?? event.data.content_index ?? 0,
					),
					status: event.type.endsWith(".done") ? "done" : "streaming",
				};
				providerSpecificFields = mergeProviderFields(providerSpecificFields, {
					openai: { responses: { reasoning_part: part } },
				});
