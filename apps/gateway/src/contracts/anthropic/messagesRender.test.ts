...
test("Messages preserves reported reasoning counts in JSON and streaming usage", async () => {
	for (const reasoningTokens of [undefined, 0, 4]) {
		const usage = {
			promptTokens: 3,
			completionTokens: 8,
			totalTokens: 11,
		...
		assert.deepEqual(response.usage.output_tokens_details, expected);
		async function* chunks(): AsyncGenerator<CanonicalChatStreamChunk> {
			yield {
				...
				usage,
				choices: [
					{ index: 0, finishReason: "stop", delta: { content: "Answer" } },
				],
			};
		}
