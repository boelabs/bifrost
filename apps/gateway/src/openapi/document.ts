		...
		paths: {
			...
			"/health/live": {
				get: {
					...
					description:
						"Always 200 while the process is responsive — including while it is draining, because a failing liveness probe restarts the container and would cut the drain short. Does NOT check Postgres/Redis, so a dependency outage never triggers a restart either. Wire this to the orchestrator's liveness probe.",
					security: [],
