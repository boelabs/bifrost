...
export class OperationLogDraft {
	...
	constructor(
		...
		// Operator traffic (master key or a dashboard session) carries no virtual key, so without this
		// its spend would be indistinguishable from unattributed usage.
		this.actor = actorOf(auth);
