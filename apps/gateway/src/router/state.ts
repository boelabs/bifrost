 ...
 *   rt:ttft_ms:{id}         EWMA of time-to-first-output
...
export interface DeploymentMetrics {
	...
	/** EWMA of time-to-first-output. Drives the adaptive first-output deadline. */
	ttftMs: number | null;
...
export interface SuccessTelemetry {
	...
	durationMs: number;
	/** Milliseconds to the first output token, when the attempt produced one. */
...
const kTtftMs = (id: string) => `rt:ttft_ms:${id}`;
...
// failures, latency, throughput, ttft) - the read side indexes into `res` using this stride.
const FIELDS_PER_DEPLOYMENT = 8;
...
export async function fetchMetrics(
	...
	ids.forEach((id, i) => {
		...
		map.set(id, {
			...
			ttftMs: typeof ttft === "string" ? Number(ttft) : null,
