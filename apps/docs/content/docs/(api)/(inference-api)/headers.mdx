...
`reasoning_effort` is the effort the served model ran at after [snapping](/docs/reasoning), or `null`
when it does not reason. The block is returned by `/v1/chat/completions`, `/v1/responses` and
`/v1/messages`.
