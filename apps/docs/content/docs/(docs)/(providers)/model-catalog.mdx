...
- A stale ladder degrades the same way. If the upstream rejects the effort the gateway sent with a
  400 that names it or lists the accepted values, the executor narrows the ladder to that list (or
  drops the rejected level), re-snaps, and retries the same deployment — at most twice. The operation
  log records it under `reasoningRecovery`; fix the catalog entry when you see one.
