...
- For custom models, `catalogEntry.operations` is required and the gateway validates per-operation
  minimums: text requires `capabilities.tools|vision|reasoning|structuredOutputs`; `strictTools` is
  optional and must only be set when the upstream guarantees schema-adherent function arguments; image requires
...
- Requests are **clamped, never rejected** for being out of range: an effort the model does not declare
  goes to the nearest declared level on the ladder, in either direction, the lower one on a tie (e.g.
  `max` → `xhigh`, `minimal` → the floor, `medium` → `high` on a `minimal`/`high` model). A positive
  effort never rounds down into `none`. Adapters send the snapped level as is, so `levels` must list
  only values the provider accepts; `gemini_level` ladders are rejected at load if they declare
  anything outside `minimal`/`low`/`medium`/`high` without an `upstreamEffortMap` entry. The only hard error is requesting reasoning on a non-reasoner. This keeps
...
- `capabilities.strictTools` is separate from `capabilities.tools`: it means the upstream guarantees
  that function-call arguments conform to the declared JSON Schema. A request with a tool whose
  `strict` is `true` is routed only to deployments that declare this capability; it is never silently
  downgraded or dropped by `unsupportedParameterStrategy`.
