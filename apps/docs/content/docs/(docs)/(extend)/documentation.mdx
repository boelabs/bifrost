...
hands out what the build produced, including the search index at `/api/search`. Queries run in
the browser. Only `/sitemap.xml` reads the deployment origin at runtime. The site has one language: English.
