...
The router selects an eligible deployment for each attempt. Deployments with the same `publicModel`
form a pool; the selected strategy determines how traffic is distributed within it. Retry rules and
fallback policies control what happens when an attempt fails.
...
|---|---:|---:|---:|---:|---:|---:|
| Text stream/WebSocket | 180s | 180s | — | 300s | 600s | 6 |
| Text JSON | 300s | — | — | 300s | 600s | 6 |
...
Reasoning models can spend minutes computing before returning text, and Chat Completions upstreams
may withhold even HTTP headers during that time. The text defaults allow this without delaying
responses that arrive sooner. `reasoningOnlyMs` is disabled by default: active reasoning remains
bounded by the total deadline. Operators can still set a separate reasoning limit or shorter idle
and first-output limits. Upgrading updates only text policies that exactly match the old defaults;
custom policies remain unchanged.
