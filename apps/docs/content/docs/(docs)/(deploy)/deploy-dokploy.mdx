---
title: "Dokploy"
description: "Deploy Bifrost on Dokploy with Docker Compose."
---

[Dokploy](https://dokploy.com) deploys the repository's `docker/compose.yaml` as a **Compose**
application.

1. **Create → Compose**, and point it at this repository (or paste the `docker/compose.yaml`).
...
4. In **Domains**, add only long-running HTTP services:
   ...
   - **dashboard** (optional), container port `3001` — its **own** host, e.g. `dash.example.com`
...
See [Operator dashboard → Serving it](/docs/dashboard#serving-it) for both settings and the two
curl commands that tell you which side is misconfigured.
