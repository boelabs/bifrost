---
title: "Portainer"
description: "Deploy Bifrost on Portainer as a Compose stack."
---

[Portainer](https://www.portainer.io) deploys the repository's `docker/compose.yaml` as a **Stack**.

1. **Stacks → Add stack**, and either paste the `docker/compose.yaml` or use the **Git repository**
...
3. **Deploy the stack.** Postgres, Redis and the one-off `migrate` job run inside the stack; the
   gateway listens on internal port `4000`, the optional dashboard on `3001` and docs on `3000`.
