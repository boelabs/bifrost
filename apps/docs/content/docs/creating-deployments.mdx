...
resource endpoint. For catalog resolution, name the Azure deployment after its model id, such as
`gpt-transcribe`; a custom Azure deployment name requires an inline `catalogEntry`. `apiVersion` is
optional (default `2024-06-01`; some transcription deployments may require a more recent one):
