...
Azure transcription uses Microsoft's
[v1 preview transcription route](https://learn.microsoft.com/azure/foundry/openai/reference-preview-latest#create-transcription),
`/openai/v1/audio/transcriptions?api-version=preview`, by default.
`upstreamModel` is the **Azure deployment name** sent in the multipart `model` field, and `baseUrl` is
the resource endpoint. For catalog resolution, name the Azure deployment after its model id, such as
`gpt-transcribe`; a custom Azure deployment name requires an inline `catalogEntry`. `apiVersion` is
optional and defaults to `preview`:
...
For an Azure resource that still needs the classic deployment-based Audio API, select the legacy
transport explicitly. Its default API version is `2024-06-01`; set `credentials.apiVersion` when the
deployment requires another version:

```json
{
  "transportOverrides": {
    "audio.transcribe": "azure_audio_transcriptions_legacy"
  },
  "credentials": {
    "apiKey": "...",
    "baseUrl": "https://my-resource.openai.azure.com",
